...
void cb_copy(Fl_Widget*, void*)
{
	Xd6HtmlPrint pd(edit->view->frame, "Docfile.html");
	pd.print(edit->view->frame);
...
void cb_bold(Fl_Widget*, void*) {
	edit->view->frame->change_style(0, FONT_BOLD);
	edit->view->redraw();
}
...
void cb_tag(Fl_Widget*, void*) {
	...
	edit->view->frame->change_style(0, A_LINK);
	e = new Xd6XmlTreeElement(NULL, 0);
	a = new Xd6XmlAttribute("href", 4);
	a->set_value("test", 4);
	e->add_attribute(a);
	e->style = edit->view->frame->cur_seg->style | DISPLAY|st;
	s = new Xd6HtmlTagA(0, e, NULL); 
	edit->view->frame->cur_seg->style &= ~st;
