...
void DemoTable::event_callback2()
{
    ...
    char *context_names[] =
        {
          "CONTEXT_NONE",       "CONTEXT_STARTPAGE",  "CONTEXT_ENDPAGE",
	  "CONTEXT_ROW_HEADER", "CONTEXT_COL_HEADER", "CONTEXT_CELL",
	  "CONTEXT_TABLE",      "CONTEXT_RC_RESIZE"
	};

    printf("'%s' callback: Row=%d Col=%d Context=%s Event=%d InteractiveResize? %d\n",
         (label() ? label() : "?"), R, C, context_names[(int)context], (int)Fl::event(),
